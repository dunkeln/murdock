...
import { issueRef, reviewActionRef } from "@/lib/server/mcp/v1/refs";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const refForItem = (item: (typeof workspace.reviewWorkItems)[number]) =>
    item.origin.sourceType === "workspace_issue"
      ? issueRef(item.id)
      : reviewActionRef(item.id);
  const reviewRefByItemId = new Map(
    workspace.reviewWorkItems.map((item) => [item.id, refForItem(item)] as const),
  );
  const itemIdByReviewRef = new Map(
    workspace.reviewWorkItems.map((item) => [refForItem(item), item.id] as const),
  );
  ...
  const activeReviewRef =
    activeItemId && reviewRefByItemId.has(activeItemId)
      ...
      : undefined;
  const planResult = await executeMurdockMcpV1Tool("get_roi_review_plan", {
    activeReviewRef,
